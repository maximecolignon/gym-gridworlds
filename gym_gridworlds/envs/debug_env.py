    ...
    def _gen_grid(self, width, height):
        ...
        for number, index, reward in zip(self.num_balls, self.balls_index, self.balls_reward):
            for i in range(number):
                self.place_obj(Ball(self.world, index, reward), top=(self.grid.width-2, self.grid.height-2), size=(1, 1))

        # Randomize the player start position and orientation
        for a in self.agents:
            self.place_agent(a,top=(0, 0), size=(2, 2))
