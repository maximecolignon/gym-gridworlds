...
class Grid:
    ...
    def process_not_seeing_own_balls(grid, mask, agent):
        for j in range(0, grid.height):
            for i in range(0, grid.width):
                obj = grid.get(i,j)
                if obj is not None:
                    if obj.type == 'ball' and obj.index == agent.index:
                        mask[i, j] = False
        return mask
...
class MultiGridEnv(gym.Env):
    ...
    #TODO: implement put_agent for non-random agent starting position
            ...
            # Move forward
                ...
                if fwd_cell is not None:
                    if fwd_cell.type == 'goal':
                        ...
                        self._reward(i, rewards, 1) #TODO : test another thing than joint_reward
                        #TODO:  change reward ?
